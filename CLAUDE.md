...
- `src/data/projects.ts` / `src/data/team.ts` — source of truth, rendered on
  the page.
...
- JSON-LD in `src/layouts/BaseLayout.astro` and `src/pages/index.astro` —
  ...
  structured data guidelines), so any visible stat (rating, NPS...) needs a
  matching JSON-LD property and vice versa.
